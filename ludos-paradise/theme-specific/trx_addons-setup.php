...
------------------------------------------------------------------------------- */
if (!function_exists('ludos_paradise_trx_addons_theme_specific_setup1')) {
	add_filter( 'trx_addons_filter_components_editor', 'ludos_paradise_trx_addons_theme_specific_components');
	function ludos_paradise_trx_addons_theme_specific_components($enable=false) {
		return LUDOS_PARADISE_THEME_FREE
					? false		// Free version
...
if (!function_exists('ludos_paradise_trx_addons_theme_specific_setup1')) {
	add_action( 'after_setup_theme', 'ludos_paradise_trx_addons_theme_specific_setup1', 1 );
	function ludos_paradise_trx_addons_theme_specific_setup1() {
		if (ludos_paradise_exists_trx_addons()) {
			add_filter( 'trx_addons_cv_enable',					'ludos_paradise_trx_addons_cv_enable');
			add_filter( 'trx_addons_demo_enable',				'ludos_paradise_trx_addons_demo_enable');
			add_filter( 'trx_addons_filter_edd_themes_market',	'ludos_paradise_trx_addons_edd_themes_market_enable');
			add_filter( 'trx_addons_cpt_list',					'ludos_paradise_trx_addons_cpt_list');
			add_filter( 'trx_addons_sc_list',					'ludos_paradise_trx_addons_sc_list');
			add_filter( 'trx_addons_widgets_list',				'ludos_paradise_trx_addons_widgets_list');
		}
	}
}

// CV
if ( !function_exists( 'ludos_paradise_trx_addons_cv_enable' ) ) {
	
	function ludos_paradise_trx_addons_cv_enable($enable=false) {
...
// Demo mode
if ( !function_exists( 'ludos_paradise_trx_addons_demo_enable' ) ) {
	
	function ludos_paradise_trx_addons_demo_enable($enable=false) {
...
// EDD Themes market
if ( !function_exists( 'ludos_paradise_trx_addons_edd_themes_market_enable' ) ) {
	
	function ludos_paradise_trx_addons_edd_themes_market_enable($enable=false) {
...
// API
if ( !function_exists( 'ludos_paradise_trx_addons_api_list' ) ) {
	
	function ludos_paradise_trx_addons_api_list($list=array()) {
...
// CPT
if ( !function_exists( 'ludos_paradise_trx_addons_cpt_list' ) ) {
	
	function ludos_paradise_trx_addons_cpt_list($list=array()) {
...
// Shortcodes
if ( !function_exists( 'ludos_paradise_trx_addons_sc_list' ) ) {
	
	function ludos_paradise_trx_addons_sc_list($list=array()) {
...
// Widgets
if ( !function_exists( 'ludos_paradise_trx_addons_widgets_list' ) ) {
	
	function ludos_paradise_trx_addons_widgets_list($list=array()) {
...
if ( !function_exists( 'ludos_paradise_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'ludos_paradise_trx_addons_menu_cache');
	function ludos_paradise_trx_addons_menu_cache($list=array()) {
...
if (!function_exists('ludos_paradise_trx_addons_localize_script')) {
	...
	function ludos_paradise_trx_addons_localize_script($arr) {
		$arr['alter_link_color'] = ludos_paradise_get_scheme_color('alter_link');
		return $arr;
	}
}
...
if ( !function_exists( 'ludos_paradise_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'ludos_paradise_trx_addons_sc_type', 10, 2);
	function ludos_paradise_trx_addons_sc_type($list, $sc) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list
        if (in_array($sc, array('trx_sc_content'))) {
            $list[ 'bordered' ] = esc_html__('Bordered', 'ludos-paradise');
            $list[ 'light' ] = esc_html__('Light', 'ludos-paradise');
        }
        if (in_array($sc, array('trx_sc_blogger'))) {
            $list[ 'horizont' ] = esc_html__('Horizontal', 'ludos-paradise');
            $list[ 'portfolio' ] = esc_html__('Portfolio', 'ludos-paradise');
        }
        if (in_array($sc, array('trx_sc_matches'))) {
            $list[ 'versus' ] = esc_html__('Versus', 'ludos-paradise');
            $list[ 'events' ] = esc_html__('Events', 'ludos-paradise');
        }
		return $list;
	}
}
...
if ( !function_exists( 'ludos_paradise_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'ludos_paradise_trx_addons_sc_atts', 10, 2);
	function ludos_paradise_trx_addons_sc_atts($atts, $sc) {
...
if ( !function_exists( 'ludos_paradise_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'ludos_paradise_trx_addons_sc_map', 10, 2);
	function ludos_paradise_trx_addons_sc_map($params, $sc) {
		...
		$param = array(
			...
			'heading' => esc_html__('Color style', 'ludos-paradise'),
			'description' => wp_kses_data( __('Select color style to decorate this block', 'ludos-paradise') ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label' => true,
			'value' => array_flip(ludos_paradise_get_list_sc_color_styles()),
			'type' => 'dropdown'
		);
...
if ( !function_exists( 'ludos_paradise_trx_addons_sow_map' ) ) {
	add_filter( 'trx_addons_sow_map', 'ludos_paradise_trx_addons_sow_map', 10, 2);
	function ludos_paradise_trx_addons_sow_map($params, $sc) {

		// Param 'color_style'
		$param = array(
			'color_style' => array(
				'label' => esc_html__('Color style', 'ludos-paradise'),
				'description' => wp_kses_data( __('Select color style to decorate this block', 'ludos-paradise') ),
				'options' => ludos_paradise_get_list_sc_color_styles(),
				'default' => 'default',
				'type' => 'select'
			)
		);
		if (in_array($sc, array('trx_sc_button')))
			ludos_paradise_array_insert_after($params, 'size', $param);
		else if (in_array($sc, array('trx_sc_action', 'trx_sc_blogger', 'trx_sc_cars', 'trx_sc_courses', 'trx_sc_content', 'trx_sc_dishes',
								'trx_sc_events', 'trx_sc_form',	'trx_sc_googlemap', 'trx_sc_portfolio', 'trx_sc_price', 'trx_sc_promo',
								'trx_sc_properties', 'trx_sc_services', 'trx_sc_team', 'trx_sc_testimonials', 'trx_sc_title',
								'trx_widget_audio', 'trx_widget_twitter')))
			ludos_paradise_array_insert_after($params, 'title_align', $param);
		return $params;
	}
}

// Add classes to the shortcode's output
if ( !function_exists( 'ludos_paradise_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'ludos_paradise_trx_addons_sc_output', 10, 4);
	function ludos_paradise_trx_addons_sc_output($output, $sc, $atts, $content) {
		...
		if (in_array($sc, array('trx_sc_action'))) {
			...
			if (!empty($atts['color_style']) && !ludos_paradise_is_inherit($atts['color_style']))
...
if ( !function_exists( 'ludos_paradise_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'ludos_paradise_trx_addons_sc_item_title_tag');
	function ludos_paradise_trx_addons_sc_item_title_tag($tag='') {
		return $tag=='h1' ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( !function_exists( 'ludos_paradise_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'ludos_paradise_trx_addons_sc_item_button_args', 10, 3);
	function ludos_paradise_trx_addons_sc_item_button_args($args, $sc, $sc_args) {
...
if ( !function_exists( 'ludos_paradise_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title',	'ludos_paradise_trx_addons_slider_title', 10, 2 );
	function ludos_paradise_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title'])) 
...
if ( !function_exists( 'ludos_paradise_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles',	'ludos_paradise_trx_addons_sc_googlemap_styles');
	function ludos_paradise_trx_addons_sc_googlemap_styles($list) {
		$list['dark'] = esc_html__('Dark', 'ludos-paradise');
		return $list;
	}
}
...
if ( !function_exists( 'ludos_paradise_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'ludos_paradise_trx_addons_tiny_mce_style_formats');
	function ludos_paradise_trx_addons_tiny_mce_style_formats($style_formats) {
		...
		if (is_array($style_formats) && count($style_formats)>0 ) {
			foreach ($style_formats as $k=>$v) {
				if ( $v['title'] == esc_html__('List styles', 'ludos-paradise') ) {
					$style_formats[$k]['items'][] = array(
								'title' => esc_html__('Square', 'ludos-paradise'),
								'selector' => 'ul',
								'classes' => 'trx_addons_list trx_addons_list_square'
							);
				}
                if ( $v['title'] == esc_html__('Inline', 'ludos-paradise') ) {
                    $style_formats[$k]['items'][] = array(
                        'title' => esc_html__('Accent hovered', 'ludos-paradise'),
                        'inline' => 'span',
...
if ( !function_exists( 'ludos_paradise_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'ludos_paradise_filter_allow_override_header_image', 'ludos_paradise_trx_addons_allow_override_header_image' );
	function ludos_paradise_trx_addons_allow_override_header_image($allow) {
		return ludos_paradise_is_team_page() ? false : $allow;
	}
}

// Get thumb size for the team items
if ( !function_exists( 'ludos_paradise_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size',	'ludos_paradise_trx_addons_thumb_size', 10, 2);
	function ludos_paradise_trx_addons_thumb_size($thumb_size='', $type='') {
//		Uncomment next lines to change team members images (default is 'avatar')
		if ($type == 'team-default')
			$thumb_size = ludos_paradise_get_thumb_size('team');
		return $thumb_size;
	}
}
...
if (!function_exists('ludos_paradise_trx_addons_override_fields')) {
	add_filter( 'trx_addons_filter_override_fields', 'ludos_paradise_trx_addons_override_fields', 10, 2);
	function ludos_paradise_trx_addons_override_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			$mb['email'] = array(
				"title" => esc_html__("E-mail",  'ludos-paradise'),
				"desc" => wp_kses_data( __("Team member's email", 'ludos-paradise') ),
				"std" => "",
				"details" => true,
				"type" => "text"
			);
