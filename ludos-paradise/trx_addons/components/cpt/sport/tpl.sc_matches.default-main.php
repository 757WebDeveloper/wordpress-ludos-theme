...
if (!empty($meta['player1']) && !empty($meta['player2'])) {
	...
	?><div class="sc_matches_item sc_matches_item_pair"><?php
		...
		?><div class="sc_matches_item_player sc_matches_item_player1"><?php
			...
			// Name or Command
			?><h3 class="sc_matches_item_player_name"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h3><?php
			// Club or Country
		...
		?><div class="sc_matches_item_score"><?php
			...
			if (!empty($meta['date_start'])) {
				?><span class="sc_matches_item_match_date"><?php echo date_i18n('H:i / M j', strtotime($meta['date_start'].(!empty($meta['time_start']) ? ' '.$meta['time_start'] : ''))); ?></span><?php
			}
			?><span class="sc_matches_item_score_value">
				<?php
					if (is_rtl())
					{
						if ( empty($meta['score']) )
			...
			if (!empty($meta['review']) && !trx_addons_is_off($meta['review'])) {
				?><a class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_matches_item_review_link theme_button', 'sc_matches', $args)); ?>" href="<?php the_permalink($meta['review']); ?>"><span class="sc_matches_item_review_title"><?php esc_html_e('Match Review', 'ludos-paradise'); ?></span></a><?php
			}
			if (!$is_match_page) {
				?></a><?php
			}
