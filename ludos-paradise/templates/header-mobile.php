 ...
 * @since LUDOS_PARADISE 1.0.39
 */

// Additional info
if (ludos_paradise_is_off(ludos_paradise_get_theme_option('header_mobile_hide_info')) && ($ludos_paradise_info=ludos_paradise_get_theme_option('header_mobile_additional_info'))!='') {
			...
			<div class="columns_wrap">
				<div class="sc_layouts_column sc_layouts_column_align_center sc_layouts_column_icons_position_left column-1_1"><?php
					?><div class="sc_layouts_item"><?php
						ludos_paradise_show_layout($ludos_paradise_info);
					?></div><!-- /.sc_layouts_item -->
	...
	<div class="content_wrap">
		<div class="columns_wrap columns_fluid">
			<div class="sc_layouts_column sc_layouts_column_align_left sc_layouts_column_icons_position_left sc_layouts_column_fluid column-1_3"><?php
				// Logo
				if (ludos_paradise_is_off(ludos_paradise_get_theme_option('header_mobile_hide_logo'))) {
					?><div class="sc_layouts_item"><?php
						set_query_var('ludos_paradise_logo_args', array('type' => 'mobile_header'));
			...
			?><div class="sc_layouts_column sc_layouts_column_align_right sc_layouts_column_icons_position_left sc_layouts_column_fluid  column-2_3"><?php
				if (ludos_paradise_exists_trx_addons()) {
					...
					if (ludos_paradise_is_off(ludos_paradise_get_theme_option('header_mobile_hide_login'))) {
						ob_start();
						do_action('ludos_paradise_action_login', array('text_login' => false, 'text_logout' => false));
						$ludos_paradise_action_output = ob_get_contents();
						ob_end_clean();
						if (!empty($ludos_paradise_action_output)) {
							?><div class="sc_layouts_item sc_layouts_menu sc_layouts_menu_default"><?php
								ludos_paradise_show_layout($ludos_paradise_action_output);
							?></div><?php
						}
					}
					// Display cart button
					if (ludos_paradise_is_off(ludos_paradise_get_theme_option('header_mobile_hide_cart'))) {
						ob_start();
						do_action('ludos_paradise_action_cart');
						$ludos_paradise_action_output = ob_get_contents();
						ob_end_clean();
						if (!empty($ludos_paradise_action_output)) {
							?><div class="sc_layouts_item"><?php
					...
					if (ludos_paradise_is_off(ludos_paradise_get_theme_option('header_mobile_hide_search'))) {
						ob_start();
						do_action('ludos_paradise_action_search', 'fullscreen', 'header_mobile_search', false);
						$ludos_paradise_action_output = ob_get_contents();
						ob_end_clean();
						if (!empty($ludos_paradise_action_output)) {
							?><div class="sc_layouts_item"><?php
